...
const resendKey=process.env.RESEND_API_KEY||(cfg&&cfg.resendKey);
if(resendKey){return r.json({configured:true,sender:cfg?.fromEmail||process.env.GMAIL_USER||"Resend API",mode:"resend"})}
if(cfg&&cfg.host&&cfg.user){return r.json({configured:true,sender:cfg.fromEmail||cfg.user,mode:"smtp"})}
const envOk=!!(process.env.GMAIL_USER&&process.env.GMAIL_APP_PASSWORD);
r.json({configured:envOk,sender:process.env.GMAIL_USER||"",mode:envOk?"smtp":"none"})}
catch(e){r.json({configured:false,sender:"",error:e.message})}});

app.get("/api/settings/email",auth,admin,async(q,r)=>{
try{let cfg={host:"",port:587,secure:false,user:"",pass:"",fromName:"",fromEmail:"",resendKey:"",configured:false};
try{const v=await store.get("_email_config");if(v){const c=JSON.parse(v);cfg={...cfg,...c,configured:!!(c.host&&c.user)||!!c.resendKey}}}catch(e){}
...
const hasResend=!!(process.env.RESEND_API_KEY||cfg.resendKey);
if(hasResend)cfg.configured=true;
r.json({config:{...cfg,pass:cfg.pass?"••••••••":"",resendKey:cfg.resendKey?"••••••••":(process.env.RESEND_API_KEY?"(env)":"")},hasResend})}
...
try{const em=await getEmailMode();
if(!em)return r.status(400).json({error:"Email not configured. Set RESEND_API_KEY env var (recommended), or configure SMTP in Settings."});
const testHtml=`<div style="font-family:Arial,sans-serif;padding:30px;max-width:500px;margin:0 auto;text-align:center"><div style="background:#0F1D3D;color:#fff;padding:20px;border-radius:10px;font-size:20px;font-weight:700;letter-spacing:.08em;margin-bottom:20px">A-BAND CONSULTING INC</div><h2 style="color:#10B981;margin-bottom:10px">Email is working!</h2><p style="color:#64748b;font-size:14px">Your outgoing mail (${em.mode.toUpperCase()}) is properly configured.</p></div>`;
if(em.mode==="resend"){
  const result=await sendViaResend(em.resendKey,em.from,em.cfg.user||em.from,"","A-Band Finance Hub — Test Email",testHtml,null,[]);
  r.json({success:true,messageId:result.id,mode:"resend"})}
else{
  await Promise.race([em.t.verify(),new Promise((_,rej)=>setTimeout(()=>rej(new Error("SMTP connection timed out (15s). Render blocks SMTP ports. Set RESEND_API_KEY env var instead — it uses HTTPS.")),15000))]);
  const info=await Promise.race([em.t.sendMail({from:em.from,to:em.cfg.user,subject:"A-Band Finance Hub — Test Email",html:testHtml}),new Promise((_,rej)=>setTimeout(()=>rej(new Error("SMTP send timed out (30s)")),30000))]);
  r.json({success:true,messageId:info.messageId,mode:"smtp"})}}
catch(e){console.error("Test email error:",e.message);r.status(500).json({error:e.message})}});

// ─── Resend API (HTTPS-based, works on Render/any host) ───
async function sendViaResend(apiKey, from, to, cc, subject, html, text, attachments) {
  ...
  if (!res.ok) throw new Error(d.message || d.error?.message || "Resend API error " + res.status);
  return d;
}

async function getEmailMode() {
  let cfg = null;
  try { const v = await store.get("_email_config"); if (v) cfg = JSON.parse(v); } catch(e) {}
  // Priority 1: Resend API key (env or saved config)
  const resendKey = process.env.RESEND_API_KEY || cfg?.resendKey;
  if (resendKey) {
    const fromEmail = cfg?.fromEmail || process.env.GMAIL_USER || "onboarding@resend.dev";
    const fromName = cfg?.fromName || "A-Band Consulting";
    return { mode: "resend", resendKey, from: `${fromName} <${fromEmail}>`, cfg: cfg || { user: fromEmail } };
  }
  // Priority 2: SMTP config (saved in DB)
  if (cfg && cfg.host && cfg.user && cfg.pass) {
    ...
    return { mode: "smtp", t, from, cfg };
  }
  // Priority 3: Gmail env vars
  if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
    ...
    return { mode: "smtp", t, from, cfg: { user: process.env.GMAIL_USER } };
  }
  return null;
}
...
const em=await getEmailMode();
if(!em)return r.status(400).json({error:"Email not configured. Set RESEND_API_KEY env var (recommended for Render), or configure SMTP in Settings.",needsConfig:true});
if(em.mode==="resend"){
  const result=await sendViaResend(em.resendKey,em.from,to,cc,subject,htmlBody,textBody,
    attachments?.map(a=>({filename:a.filename,content:a.content}))||[]);
  r.json({success:true,messageId:result.id})}
else{
  const mailOpts={from:em.from,to,subject,html:htmlBody||undefined,text:textBody||undefined};
  ...
  const info=await Promise.race([em.t.sendMail(mailOpts),new Promise((_,rej)=>setTimeout(()=>rej(new Error("SMTP timed out (30s). Try setting RESEND_API_KEY env var instead — it uses HTTPS and works on Render.")),30000))]);
