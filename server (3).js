...
let cfg={host:"",port:587,secure:false,user:"",pass:"",fromName:"",fromEmail:"",configured:false};
...
const{host,port,secure,user,pass,fromName,fromEmail}=q.body;
...
if(!transport)return r.status(400).json({error:"Email not configured"});
try{await Promise.race([transport.t.verify(),new Promise((_,rej)=>setTimeout(()=>rej(new Error("Connection timed out after 15 seconds. Render may block SMTP port 587/465. Try port 2525 or use environment variables instead.")),15000))]);
const info=await Promise.race([transport.t.sendMail({from:transport.from,to:transport.cfg.user,subject:"A-Band Finance Hub — Test Email",html:`<div style="font-family:Arial,sans-serif;padding:30px;max-width:500px;margin:0 auto;text-align:center"><div style="background:#0F1D3D;color:#fff;padding:20px;border-radius:10px;font-size:20px;font-weight:700;letter-spacing:.08em;margin-bottom:20px">A-BAND CONSULTING INC</div><h2 style="color:#10B981;margin-bottom:10px">Email is working!</h2><p style="color:#64748b;font-size:14px">Your outgoing mail server is properly configured.</p></div>`}),new Promise((_,rej)=>setTimeout(()=>rej(new Error("Send timed out after 30s")),30000))]);
...
const{to,cc,subject,htmlBody,textBody,attachments}=q.body;
...
const info=await Promise.race([transport.t.sendMail(mailOpts),new Promise((_,rej)=>setTimeout(()=>rej(new Error("SMTP send timed out after 30s. Your hosting provider may block outbound SMTP.")),30000))]);
r.json({success:true,messageId:info.messageId})}
