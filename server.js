...
// ─── Email ───
app.get("/api/email/config",auth,(q,r)=>{r.json({configured:!!(process.env.GMAIL_USER&&process.env.GMAIL_APP_PASSWORD),sender:process.env.GMAIL_USER||""})});
...
if(!gmailUser||!gmailPass)return r.status(400).json({error:"Email not configured. Set GMAIL_USER and GMAIL_APP_PASSWORD environment variables.",needsConfig:true});
try{
const transporter=nodemailer.createTransport({service:"gmail",auth:{user:gmailUser,pass:gmailPass}});
const mailOpts={from:`"${q.session.user.displayName||"A-Band Consulting"}" <${gmailUser}>`,to,subject,html:htmlBody||undefined,text:textBody||undefined};
